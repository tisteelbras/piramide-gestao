...
const STATUS_LABEL: Record<string, string> = { nao_iniciada: "Não iniciada", em_andamento: "Em andamento", revisada: "Revisada" };
    ...
    <div style={{ background: "#fff", color: "#0e1a24", minHeight: "100vh" }}>
      ...
      <div className="rel-wrap">
        ...
        <header style={{ borderBottom: "3px solid #0068a9", paddingBottom: 18, marginBottom: 24 }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: "#0068a9" }}>Steel<span style={{ color: "#47ad4b" }}>bras</span></div>
          <h1 style={{ fontSize: 28, fontWeight: 800, margin: "8px 0 2px" }}>Diagnóstico de Maturidade — {rel.setor}</h1>
        ...
        <section style={{ display: "flex", gap: 20, alignItems: "center", background: "#f4f8fb", borderRadius: 14, padding: 20, marginBottom: 26 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 14, color: "#46586a", lineHeight: 1.5 }}>
              O setor <b>{rel.setor}</b> apresenta maturidade geral de <b>{Math.round(rel.geral)}%</b> ({rel.grau}), consolidando os quatro níveis da pirâmide de gestão.
            </p>
          </div>
        </section>
        ...
        {rel.niveis.map((n) => (
          <section key={n.id} className="rel-nivel" style={{ marginBottom: 22, borderLeft: `4px solid ${n.cor}`, paddingLeft: 16 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ marginLeft: "auto", fontSize: 13, fontWeight: 800, color: n.cor }}>{Math.round(n.preenchimento)}% · {n.grau}</span>
            </div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <tbody>
                {n.criterios.map((c, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #eef2f6" }}>
                    <td style={{ padding: "6px 4px", color: "#46586a" }}>{c.titulo}</td>
                    <td style={{ padding: "6px 4px", textAlign: "right", width: 90, color: "#8493a0", fontSize: 12 }}>{STATUS_LABEL[c.status]}</td>
                    <td style={{ padding: "6px 4px", textAlign: "right", width: 50, fontWeight: 800, color: n.cor, fontVariantNumeric: "tabular-nums" }}>{c.nota ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <footer style={{ marginTop: 40, paddingTop: 14, borderTop: "1px solid #e3ebf1", fontSize: 11.5, color: "#8493a0", textAlign: "center" }}>
          SOS — Sistema de Operação Steelbras · Relatório gerado automaticamente
        </footer>
      </div>
    </div>
  );
